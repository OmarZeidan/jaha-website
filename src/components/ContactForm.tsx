...
function TextAreaInput({
  ...
  return (
    <div className="group relative z-0 transition-all focus-within:z-10">
      ...
      <label
        htmlFor={id}
        className="pointer-events-none absolute top-0 left-0 mx-6 w-full origin-left bg-white/80 px-1 pt-4 text-base/6 text-neutral-500 transition-all duration-200 peer-not-placeholder-shown:scale-75 peer-not-placeholder-shown:font-semibold peer-not-placeholder-shown:text-neutral-950 peer-focus:scale-75 peer-focus:font-semibold peer-focus:text-neutral-950"
      >
        {label}
      </label>
    </div>
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        setSubmitMessage(
          result.error || 'Sorry, there was an error sending your message.',
        )
        setIsSuccess(false)
      }
    } catch (error) {
